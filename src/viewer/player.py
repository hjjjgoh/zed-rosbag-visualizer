    ...
    with Reader(bag_path) as reader:
        ...
        for conn, timestamp, rawdata in reader.messages(conns):
            ...
            if conn.topic == image_topic:
                ...
                rr.log("world/camera/image/rgb_left", rr.Image(img_rgb))
            ...
            # ───────── Right ─────────
                ...
                img_R = msg_to_rgb_numpy(msg)       # ← 통일
                rr.log("image/rgb_right", rr.Image(img_R)) 
                ...
                if matched_left is not None:
                    ...
                    rr.log("image/depth_foundation", rr.Image(color_rgb))
                    rr.log(
            ...
            # visualization odometry data
                ...
                """
                # 데이터를 player.json으로 저장하기 위해 수집
                player_trajectory.append({
                    "bag_timestamp_ns": timestamp,
                    "ros_timestamp_ns": ros_time_ns(msg),
                    "position": {
                    ...
                    "relative_position": t_adj.tolist()
                })
                """

    # 수집된 오도메트리 데이터를 player.json 파일로 저장
    if player_trajectory:
        output_path = bag_path.parent / f"{bag_path.stem}_player.json"
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(player_trajectory, f, ensure_ascii=False, indent=2)
        print(f"\n[INFO] Saved {len(player_trajectory)} odometry points from player to: {output_path}")

    # traj 변수(상대적 궤적)를 traj.json 파일로 저장
    if traj:
        # NumPy array를 JSON으로 저장하기 위해 리스트로 변환
        traj_to_save = [t.tolist() for t in traj]
        output_path_traj = bag_path.parent / f"{bag_path.stem}_traj.json"
        with open(output_path_traj, "w", encoding="utf-8") as f:
            json.dump(traj_to_save, f, ensure_ascii=False, indent=2)
        print(f"[INFO] Saved {len(traj_to_save)} trajectory points from 'traj' to: {output_path_traj}")