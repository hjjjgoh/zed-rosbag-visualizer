...
class DisparityEstimator:
    def __init__(self, weights_path: str):
        ...
        try:
            ...
            self.logger.debug("Safe checkpoint load (weights_only=True) failed.")
            self.logger.debug("Falling back to unsafe load (weights_only=False). Use only if checkpoint is trusted.")
            ckpt = torch.load(weights_path, map_location="cpu", weights_only=False)
            self.logger.debug("Checkpoint loaded with weights_only=False")
        ...
        if isinstance(ckpt, dict) and "model" in ckpt and isinstance(ckpt["model"], dict):
            ...
            raise RuntimeError(f"Unexpected checkpoint structure: {type(ckpt)}")
        ...
        try:
            ...
            self.logger.debug("Model state_dict loaded successfully (strict=True).")
...
class DepthFoundationStereo:
    def __init__(self, weights_path: str):
        ...
        try:
            torch.serialization.add_safe_globals([np.core.multiarray.scalar])  # PyTorch 2.4+ 만 작동 (없으면 무시)
    ...
    def colorize_disparity(self, disp: np.ndarray) -> np.ndarray:
        ...
        if np.any(valid):
            dmin = float(np.percentile(disp[valid], 2)); dmax = float(np.percentile(disp[valid], 98))
            if dmax <= dmin: dmin, dmax = float(np.min(disp[valid])), float(np.max(disp[valid]))
        else:
            dmin, dmax = 0.0, 1.0
    ...
    def run_pair_np(self, left_rgb: np.ndarray, right_rgb: np.ndarray, fx: float, baseline_m: float, return_vis: bool = True):
        tL = self.preprocess(left_rgb); tR = self.preprocess(right_rgb)
        ...
        if return_vis: out["disparity_rgb"] = self.colorize_disparity(disp)
        return out

    def run_pair_msgs(self, left_msg, right_msg, fx: float, baseline_m: float, return_vis: bool = True):
        left_rgb = msg_to_rgb_numpy(left_msg); right_rgb = msg_to_rgb_numpy(right_msg)
